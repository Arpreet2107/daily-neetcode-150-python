...
# Time complexity: O(nlogn) due to the set lookup and insertion operations, which on average are O(1) but can be O(n) in the worst case. However, since we are only doing a single pass through the list, the overall time complexity is O(n).
